...
def get_all_passes():
    ...
    try:
        ...
        pass
    # Fallback / mock mode
    return {"data": mock_store.get_all_passes(), "source": "standalone_engine"}
...
def get_pass_by_number(pass_number: str):
    ...
    try:
        if db_manager.get_driver():
            ...
            if results:
                return {"data": results[0], "source": "cognoDB"}
    ...
    if not matched:
        raise HTTPException(status_code=404, detail="Pass not found")
    return {"data": matched, "source": "standalone_engine"}
...
def create_pass(req: CreatePassRequest):
    ...
    try:
        if db_manager.get_driver():
            ...
            return {"message": "Pass issued successfully", "data": res[0] if res else params, "source": "cognoDB"}
    except Exception as e:
        pass
    ...
    return {"message": "Pass issued successfully", "data": created, "source": "standalone_engine"}
...
def update_pass_status(pass_id: str, req: UpdateStatusRequest):
    ...
    try:
        if db_manager.get_driver():
            ...
            if res:
                return {"message": f"Pass status updated to {req.status}", "data": res[0], "source": "cognoDB"}
    except Exception:
        pass
    ...
    if not updated:
        raise HTTPException(status_code=404, detail="Pass not found")
    return {"message": f"Pass status updated to {req.status}", "data": updated, "source": "standalone_engine"}
